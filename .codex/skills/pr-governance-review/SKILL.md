...
4. Use the async PR-train method as the default for more than one PR: identify all trains, run non-touching trains in parallel through evidence lanes, sequence touching PRs oldest-first inside each train, queue independent green PRs together, run disjoint patch trains, and run decision waves asynchronously.
