...
- KYC owns the ADK drafting contract for approved replies, but strict client-side ZK mode applies that contract in the browser. Backend ADK/LLM paths must not receive decrypted PKM plaintext unless the strict-ZK contract is explicitly redesigned.
